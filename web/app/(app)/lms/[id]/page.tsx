...
import { ArrowLeft, CalendarClock, CheckCircle2, XCircle, Clock, Calendar, FileText, AlertCircle, Stethoscope, User } from 'lucide-react'
...
const STATUS_META = {
  pending:  { label: 'Pending Approval', variant: 'warning'     as const, icon: Clock,        color: 'text-yellow-600 bg-yellow-50 border-yellow-200' },
  approved: { label: 'Approved',         variant: 'success'     as const, icon: CheckCircle2, color: 'text-green-600  bg-green-50  border-green-200'  },
  rejected: { label: 'Rejected',         variant: 'destructive' as const, icon: XCircle,      color: 'text-red-600    bg-red-50    border-red-200'    },
}
const TYPE_META = {
      ...
      <main className="flex-1 p-6 max-w-2xl space-y-5 animate-fade-in">
        ...
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${s.color}`}>
          ...
          <div className="flex-1">
            ...
            {leave.managerNote && <p className="text-xs mt-0.5">Note: {leave.managerNote}</p>}
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-surface border border-surface-border">
              <p className="text-[10px] text-surface-muted uppercase tracking-wide font-semibold mb-1">Reason</p>
              <p className="text-sm text-gray-900">{leave.reason}</p>
            </div>

            <p className="text-xs text-surface-muted">
              Submitted {new Date(leave.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
            </p>
          </CardContent>
        </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                <Stethoscope className="w-4 h-4 text-red-500" /> Medical Documents
              </CardTitle>
            </CardHeader>
            <CardContent>
              {leave.medicalDocs && leave.medicalDocs.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {leave.medicalDocs.map((url, i) => (
                    <a key={i} href={url} target="_blank" rel="noreferrer"
                      className="flex items-center gap-2 p-3 rounded-xl border border-red-200 bg-red-50 hover:bg-red-100 transition-all group">
                      <FileText className="w-5 h-5 text-red-400 flex-shrink-0" />
                      <span className="text-xs font-semibold text-red-700 truncate">Document {i + 1}</span>
                    </a>
                  ))}
                </div>
              ) : (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-amber-50 border border-amber-200">
                  ...
                  <p className="text-xs font-semibold text-amber-700">No medical documents uploaded — verify with employee before approving.</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {isManager && leave.status === 'pending' && (
          <Card>
            ...
            <CardContent>
              {leave.type === 'medical' && (!leave.medicalDocs || leave.medicalDocs.length === 0) && (
